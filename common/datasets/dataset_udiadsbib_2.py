...
def class_aware_training_transform(image, mask_class, patch_size, num_classes=6, use_aggressive_aug=False):
    ...
    if has_very_rare_class:
        ...
        color_prob = 0.0
        affine_prob = 0.0
        rotation_range = (-15, 15)  # Maintain some augmentation for common classes
    ...
    def __getitem__(self, idx):
        if not self.use_patched_data and self.patch_size is not None and self.split == 'training':
            ...
            if self.model_type and self.model_type.lower() == 'sstrans':
                ...
                # Apply ImageNet normalization for models with ImageNet-pretrained encoders
                # Network model uses EfficientNet-B4 encoder (pretrained on ImageNet)
                # SwinUnet and SimMIM use Swin Transformer encoder (pretrained on ImageNet)
                # Note: 'network' is the new name for the model previously called 'hybrid1'
                # Hybrid2 uses Swin encoder, so it doesn't need ImageNet normalization
                if self.model_type and self.model_type.lower() in ['network', 'swinunet', 'simmim']:
            ...
            mask = Image.open(mask_path).convert("RGB")
            mask_class = rgb_to_class(np.array(mask), self.num_classes)
            # Keep original dimensions for inference - no fixed resize
            # test.py handles variable image sizes via patch stitching at the correct patch size
            # Resizing to fixed size (2016x1344) distorts aspect ratios and creates incorrect predictions
