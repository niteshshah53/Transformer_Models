...
def default_transform(image, mask_class, img_size=(448, 448)):
    """Default transform with resize and basic augmentations."""
    # Resize
    ...
    def __getitem__(self, idx):
        if not self.use_patched_data and self.patch_size is not None and self.split == 'training':
            ...
            if self.model_type and self.model_type.lower() == 'sstrans':
                ...
                # CRITICAL: Apply ImageNet normalization for SwinUNet
                # SwinUNet uses pretrained Swin Transformer weights trained on ImageNet
                # Without this normalization, the model receives wrong input distribution
                if self.model_type and self.model_type.lower() == 'swinunet':
