...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Test MissFormer model on historical document datasets',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Test on U-DIADS-Bib dataset with MissFormer
  python test.py --model missformer --output_dir ./models/ \\
                 --dataset UDIADS_BIB --manuscript Latin2 --is_savenii
  
  # Test on DIVAHISDB dataset with MissFormer
  python test.py --model missformer --dataset DIVAHISDB \\
    ...
    parser.add_argument('--cfg', type=str, required=False, metavar="FILE",
                       help='Path to model configuration file (optional for MissFormer)')
    parser.add_argument('--output_dir', type=str, required=True,
                       help='Directory containing trained model checkpoints')
    
    # Model selection
    parser.add_argument('--model', type=str, default='missformer',
...
def get_model(args, config=None):
    """
    Create and load the MissFormer model.
    
    Args:
        args: Command line arguments
        config: Model configuration (not used for MissFormer)
